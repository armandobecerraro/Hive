...
impl HiveOrchestrator {
    ...
    pub async fn start(
        ...
        let hive_state = std::sync::Arc::new(Mutex::new(crate::state::HiveState::load_or_new(&target)?));
        {
            ...
            st.log_decision(
                ...
                format!(
                    ...
                    self.profile.languages, specialists.len()
                ),
            );
            st.save(&target)?;
        }
...
pub fn system_allows_new_instance(policy: &ResourcePolicy) -> bool {
    ...
    let ram_ok =
        policy.min_available_ram_bytes == 0 || avail > policy.min_available_ram_bytes;
...
impl Orchestrator {
    ...
    pub async fn incubate_worker_instance(self, task: WorkerTask, state: ArcMutexState) -> Result<()> {
    ...
    loop {
        ...
        if legacy_orphan {
            ...
            #[cfg(feature = "multiagent")]
            {
                ...
                let llm_client: Box<dyn crate::brain::LLMClient> =
                    if std::env::var("HIVE_USE_MOCK_LLM").unwrap_or_default() == "1" {
                ...
                let blackboard =
                    std::sync::Arc::new(crate::blackboard::Blackboard::new());
        ...
        let review: MaintainerReview = rx.await.map_err(|_| anyhow!("sin respuesta del Consejo"))?;
...
fn checkout_branch(repo: &Repository, name: &str) -> Result<()> {
    ...
    repo.checkout_head(Some(
        git2::build::CheckoutBuilder::default().force().remove_ignored(false),
    ))?;
    Ok(())
}
...
fn ensure_mainline_seed(repo_root: &Path) -> Result<()> {
    ...
    let tree_id = index.write_tree()?;
    let tree = repo.find_tree(tree_id)?;
    let oid = repo.commit(Some("HEAD"), &sig, &sig, "chore: seed main (Queen)", &tree, &[])?;
...
fn merge_branch_into_main(
    ...
    match branch_mode {
        WorkerBranchMode::DerivedFromMain => merge_derived_worker_into_main(repo_root, worker_branch),
...
fn merge_derived_worker_into_main(repo_root: &Path, worker_branch: &str) -> Result<()> {
    ...
    crate::git_manager::delete_local_feature_branch_after_integrated_merge(repo_root, worker_branch)?;
...
mod tests {
    ...
    async fn obrera_descartada_tras_max_rechazos_vuelve_a_main_y_archiva_log() {
        ...
        let st = std::sync::Arc::new(Mutex::new(HiveState::default()));
        run_worker_queue(orch, vec![task], st)
            .await
            .unwrap();
    ...
    fn merge_derived_falla_si_hay_conflicto() {
        ...
        repo
            .commit(Some("HEAD"), &sig, &sig, "w", &tree2, &[&head])
            .unwrap();
        ...
        repo
            .commit(Some("HEAD"), &sig, &sig, "m", &tree3, &[&head_m])
            .unwrap();
    ...
    async fn read_version_string_ok() {
        ...
        .unwrap();
        assert_eq!(
            read_version_string(tmp.path()).await.unwrap(),
            "9.8.7"
        );
    }
