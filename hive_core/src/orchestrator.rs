...
impl HiveOrchestrator {
    ...
    pub async fn start(
        ...
        let mut specialists = self.profile.specialists.clone();
        let actionable = std::hint::black_box(hive_request.is_actionable());
        if matches!(work_mode, HiveWorkMode::Improve) && !actionable {
            specialists = crate::discovery::focus_specialists_for_improve_idle_repo(
                &self.profile,
                specialists,
            );
            info!(
                n = specialists.len(),
                keys = ?specialists.iter().map(|s| s.language_key.as_str()).collect::<Vec<_>>(),
                "mejora sin pedido en hive.request: especialistas enfocados al stack (sin ruido ext_*)"
            );
        }
    ...
    loop {
        ...
        if legacy_orphan {
            ...
            #[cfg(feature = "multiagent")]
            {
                ...
                let mock_explicit =
                    std::env::var("HIVE_USE_MOCK_LLM").unwrap_or_default() == "1";
                let ollama_url = std::env::var("OLLAMA_BASE_URL")
                    .ok()
                    .filter(|u| !u.trim().is_empty());

                let llm_client: Option<Box<dyn crate::brain::LLMClient>> = if mock_explicit {
                    Some(Box::new(MockLLMClient::new(
                        "[FILE: src/generated.rs]\n[ACTION: create]\n[CODE]\n// Generated by The Hive\nfn placeholder() {}\n[/CODE]".to_string(),
                    )))
                } else if let Some(ref url) = ollama_url {
                    let model =
                        std::env::var("OLLAMA_MODEL").unwrap_or_else(|_| "codellama".to_string());
                    Some(Box::new(crate::brain::OllamaClient::new(url, &model)))
                } else if let Some(c) = crate::brain::OpenAiCompatibleClient::try_from_env() {
                    Some(Box::new(c))
                } else {
                    None
                };
                ...
                if let Some(llm_client) = llm_client {
                    ...
                    info!(
                        specialist = %task.specialist.language_key,
                        "WorkerAgent sin Brain LLM (OLLAMA_BASE_URL, o HIVE_OPENAI_BASE_URL+HIVE_OPENAI_API_KEY+HIVE_OPENAI_MODEL, o HIVE_USE_MOCK_LLM=1)"
                    );
                }
            }
        ...
        if run_tests_before_mr {
            let rr = repo_root.clone();
            tokio::task::spawn_blocking(move || crate::repo_checks::preflight_tests_before_mr(&rr))
                .await
                .context("join preflight tests (cargo / flutter)")??;
        }
