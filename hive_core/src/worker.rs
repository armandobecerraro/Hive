...
impl Worker {
    ...
    async fn process_task(&self, task: &Task) -> Result<()> {
        ...
        Err(anyhow::anyhow!("Task failed after {} retries", self.max_retries))
    }
    ...
    fn git_add_and_commit(&self, changes: &[FileChange], task: &Task, branch_name: &str) -> Result<()> {
    ...
    fn parse_code_response(&self, response: &str, task: &Task) -> Result<Vec<FileChange>> {
        Ok(crate::brain::parse_llm_code_response(response, &task.target_file))
    }
...
pub async fn spawn_workers(
    ...
    for i in 0..count {
        let specialist = specialists.get(i % specialists.len()).cloned()
