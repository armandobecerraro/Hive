...
impl HiveDashboard {
    ...
    pub async fn register_worker(&self, worker_id: Uuid, specialist_key: String) {
        ...
        workers.insert(worker_id, WorkerStatus {
            id: worker_id,
            specialist_key,
    ...
    pub async fn update_worker_status(&self, worker_id: Uuid, status: WorkerState, task: Option<String>) {
        let mut workers = self.workers.write().await;
    ...
    pub async fn add_event(&self, event: String) {
        ...
        events.push(format!("[{}] {}", chrono::Utc::now().format("%H:%M:%S"), event));
    ...
    pub async fn get_snapshot(&self, pending: usize, in_progress: usize, completed: usize) -> HiveSnapshot {
        let workers = self.workers.read().await;
        let events = self.events.read().await;
        let quality = self.quality.read().await;

        let active_workers = workers.values().filter(|w| matches!(w.status, WorkerState::Working)).count();
        let idle_workers = workers.values().filter(|w| matches!(w.status, WorkerState::Idle)).count();
        let failed_workers = workers.values().filter(|w| matches!(w.status, WorkerState::Failed(_))).count();
    ...
    pub async fn format_status(&self, pending: usize, in_progress: usize, completed: usize) -> String {
        ...
        output.push_str(&format!("  Obreras: {} total, {} activas, {} inactivas, {} fallidas\n",
            snapshot.total_workers, snapshot.active_workers, snapshot.idle_workers, snapshot.failed_workers));
        output.push_str(&format!("  Tareas: {} pendientes, {} en progreso, {} completadas\n\n",
        ...
        for worker in &snapshot.workers {
            ...
            output.push_str(&format!("  {} {} ({}) - {} completadas, {} fallidas\n",
                status_icon, worker.specialist_key, &worker.id.to_string()[..8],
...
mod tests {
    ...
    async fn test_dashboard_workflow() {
        ...
        dashboard.register_worker(worker_id, "rust".to_string()).await;
        
        dashboard.update_worker_status(worker_id, WorkerState::Working, Some("Tarea 1".to_string())).await;
        dashboard.record_task_completed(worker_id).await;
        
        dashboard.add_event("Obrera completó tarea".to_string()).await;
    ...
    async fn test_format_status() {
        ...
        let worker_id = Uuid::new_v4();
        dashboard.register_worker(worker_id, "python".to_string()).await;
